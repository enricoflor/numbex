...
with ~\ref{ex:<label>}~ (numbex delimiters are ignored to make sure the
LaTeX output is syntactically correct).  To convert in the other
direction, use ~numbex-convert-from-latex~.
